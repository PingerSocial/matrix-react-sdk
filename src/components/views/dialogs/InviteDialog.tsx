        ...
        return (
            <span className="mx_InviteDialog_userTile">
                <span className="mx_InviteDialog_userTile_pill">
                    {avatar}
                    <span className="mx_InviteDialog_userTile_name">
...
class DMRoomTile extends React.PureComponent<IDMRoomTileProps> {
    ...
    _highlightName(str: string) {
        ...
        while ((ii = lowerStr.indexOf(filterStr, i)) >= 0) {
            ...
            if (ii > i) {
                ...
                result.push(
                    <span key={i + "begin"}>{str.substring(i, ii)}</span>,
                );
            }
    ...
    render() {
        ...
        if (this.props.lastActiveTs) {
            ...
            timestamp = (
                <span className="mx_InviteDialog_roomTile_time">{humanTs}</span>
            );
        }
...
> {
    ...
    constructor(props) {
        ...
        if (props.kind === KIND_INVITE && !props.roomId) {
            throw new Error(
                "When using KIND_INVITE a roomId is required for an InviteDialog",
            );
        } else if (props.kind === KIND_CALL_TRANSFER && !props.call) {
            throw new Error(
                "When using KIND_CALL_TRANSFER a call is required for an InviteDialog",
            );
        }
        ...
        if (props.roomId) {
            ...
            if (!room) {
                throw new Error(
                    "Room ID given to InviteDialog does not look like a room",
                );
            }
            room.getMembersWithMembership("invite").forEach((m) =>
                alreadyInvited.add(m.userId),
            );
            room.getMembersWithMembership("join").forEach((m) =>
                alreadyInvited.add(m.userId),
            );
            // add banned users, so we don't try to invite them
            room.getMembersWithMembership("ban").forEach((m) =>
                alreadyInvited.add(m.userId),
            );
    ...
    static buildRecents(
        ...
        const dmTaggedRooms =
            RoomListStore.instance.orderedLists[DefaultTagID.DM] || [];
        const myUserId = MatrixClientPeg.get().getUserId();
        for (const dmRoom of dmTaggedRooms) {
            const otherMembers = dmRoom
                .getJoinedMembers()
            ...
            for (const member of otherMembers) {
                ...
                console.warn(
                    `Adding DM room for ${member.userId} as ${dmRoom.roomId} from tag, not DM map`,
                );
        ...
        for (const userId in rooms) {
            ...
            if (excludedTargetIds.has(userId)) {
                console.warn(
                    `[Invite:Recents] Excluding ${userId} from recents`,
                );
                continue;
            }
            ...
            if (!member) {
                ...
                console.warn(
                    `[Invite:Recents] ${userId} is missing a member object in their own DM (${room.roomId})`,
                );
                continue;
            }
            ...
            if (!lastEventTs) {
                ...
                console.warn(
                    `[Invite:Recents] ${userId} (${room.roomId}) has a weird last timestamp: ${lastEventTs}`,
                );
                continue;
            }
    ...
    _buildSuggestions(
        ...
        const joinedRooms = MatrixClientPeg.get()
            .getRooms()
            .filter(
                (r) =>
                    r.getMyMembership() === "join" &&
                    r.getJoinedMemberCount() <= maxConsideredMembers,
            );
        ...
        const memberRooms = joinedRooms.reduce((members, room) => {
            ...
            const joinedMembers = room
                .getJoinedMembers()
                .filter((u) => !excludedTargetIds.has(u.userId));
            for (const member of joinedMembers) {
                ...
                if (
                    ...
                    members[member.userId].pickedMemberRoomSize
                ) {
        ...
        const memberScores = Object.values(memberRooms).reduce(
            (scores, entry: { member: RoomMember; rooms: Room[] }) => {
                const numMembersTotal = entry.rooms.reduce(
                    (c, r) => c + r.getJoinedMemberCount(),
                    0,
                );
                ...
                scores[entry.member.userId] = {
                    ...
                    score: Math.max(
                        0,
                        Math.pow(1 - numMembersTotal / maxRange, 5),
                    ),
                };
                return scores;
            },
        ...
        const trueJoinedRooms = MatrixClientPeg.get()
            .getRooms()
        ...
        for (const room of trueJoinedRooms) {
            ...
            for (
                ...
                i >= Math.max(0, events.length - maxMessagesConsidered);
                i--
            ) {
                const ev = events[i];
                ...
                if (
                    !lastSpoke[ev.getSender()] ||
                    lastSpoke[ev.getSender()] < ev.getTs()
                ) {
                    lastSpoke[ev.getSender()] = ev.getTs();
                    lastSpokeMembers[ev.getSender()] = room.getMember(
                        ev.getSender(),
                    );
                }
            }
    ...
    _startDm = async () => {
        ...
        if (targetIds.length === 1) {
            ...
            existingRoom = DMRoomMap.shared().getDMRoomForIdentifiers(
                targetIds,
            );
        }
        if (existingRoom) {
        ...
        if (privateShouldBeEncrypted()) {
            ...
            const has3PidMembers = targets.some(
                (t) => t instanceof ThreepidMember,
            );
            if (!has3PidMembers) {
    ...
    _inviteUsers = () => {
        ...
        inviteMultipleToRoom(this.props.roomId, targetIds)
            .then((result) => {
                CountlyAnalytics.instance.trackSendInvite(
                    startTime,
                    this.props.roomId,
                    targetIds.length,
                );
    ...
    _transferCall = async () => {
        ...
        if (targetIds.length > 1) {
            this.setState({
                errorText: _t(
                    "A call can only be transferred to a single user.",
                ),
            });
        }
    ...
    _onKeyDown = (e) => {
        ...
        if (
            ...
            e.preventDefault();
            this._removeMember(
                this.state.targets[this.state.targets.length - 1],
            );
    ...
    _updateSuggestions = async (term) => {
        MatrixClientPeg.get()
            ...
            .then(async (r) => {
                ...
                try {
                    ...
                    console.warn(
                        "Non-fatal error trying to make an invite for a user ID",
                    );
                    console.warn(e);
        ...
        if (
            ...
            this.setState({
                ...
                threepidResultsMixin: [
                    { user: new ThreepidMember(term), userId: term },
                ],
            });
            try {
    ...
    _showMoreSuggestions = () => {
        this.setState({
            numSuggestionsShown:
                this.state.numSuggestionsShown + INCREMENT_ROOMS_SHOWN,
        });
    };
    ...
    _renderSection(kind: "recents" | "suggestions") {
        let sourceMembers =
            kind === "recents" ? this.state.recents : this.state.suggestions;
        let showNum =
            kind === "recents"
                ? this.state.numRecentsShown
                : this.state.numSuggestionsShown;
        const showMoreFn =
            kind === "recents"
                ? this._showMoreRecents.bind(this)
                : this._showMoreSuggestions.bind(this);
        const lastActive = (m) => (kind === "recents" ? m.lastActive : null);
        let sectionName =
            kind === "recents" ? _t("Recent Conversations") : _t("Suggestions");
        ...
        if (
            ...
            sectionSubname = _t(
                "May include members not in %(communityName)s",
                { communityName },
            );
        }
        ...
        if (this.state.filterText && hasMixins && kind === "suggestions") {
            ...
            const notAlreadyExists = (u: any): boolean => {
                return (
                    ...
                    !priorityAdditionalMembers.some(
                        (m) => m.userId === u.userId,
                    ) &&
            ...
            otherAdditionalMembers = this.state.serverResultsMixin.filter(
                notAlreadyExists,
            );
            priorityAdditionalMembers = this.state.threepidResultsMixin.filter(
                notAlreadyExists,
            );
        }
        const hasAdditionalMembers =
            priorityAdditionalMembers.length > 0 ||
            otherAdditionalMembers.length > 0;
        ...
        if (this.state.filterText) {
            ...
            sourceMembers = sourceMembers.filter(
                (m) =>
                    m.user.name.toLowerCase().includes(filterBy) ||
                    m.userId.toLowerCase().includes(filterBy),
            );
        ...
        const tiles = toRender.map((r) => (
            <DMRoomTile
                ...
                isSelected={this.state.targets.some(
                    (t) => t.userId === r.userId,
                )}
            />
        ));
        return (
            <div className="mx_InviteDialog_section">
                <h3>{sectionName}</h3>
                {sectionSubname ? (
                    <p className="mx_InviteDialog_subname">{sectionSubname}</p>
                ) : null}
                {tiles}
                {showMore}
            </div>
                        ...
                        },
                        {
                            default: (sub) => (
                                <a
                                    href="#"
                                    onClick={
                                        this._onUseDefaultIdentityServerClick
                                    }
                                >
                                    {sub}
                                </a>
    ...
    render() {
        ...
        const identityServersEnabled = SettingsStore.getValue(
        ...
        if (this.props.kind === KIND_DM) {
            ...
            helpText = _t(
                "Start a conversation with someone using their username (like <userId/>).",
                {},
                {
                    userId: () => {
                        return (
            ...
            if (identityServersEnabled) {
                helpText = _t(
                    ...
                    {},
                    {
                        ...
                        a: (sub) => (
                            <a
                                href={makeRoomPermalink(this.props.roomId)}
                                rel="noreferrer noopener"
                                target="_blank"
                            >
                                {sub}
